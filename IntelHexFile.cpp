...
*/
#if 0
bool BinToHexFile(const wstring& filePath, const vector<unsigned char>& vBuffer)
{
...
// -anderson_variable 06/10/11 ///////////////////////////////////////
    ...
    int iULBA = static_cast<unsigned int>((vBuffer.size() >> 16) & 0xFFFF);
    unsigned int idx = 0;
    ...
    do //for(unsigned int idx = 0; idx < iULBA; ++idx)
    {
        ...
        sprintf(buf, ":02000004%04X%02X\n", idx, static_cast<unsigned char>(0x100 - 0x06 - (idx & 0xFF) - ((idx>>8) & 0xFF)));
        out << buf ;

        size_t len = iULBA ? 0x10000 : (vBuffer.size() & 0xFFFF);
        while(len > 0)
        {
            one_record.intel_lg_data = ( LL_MAX_LINE > len ) ? len : LL_MAX_LINE ;
            copy(vBuffer.begin() + one_record.intel_adr + (idx<<16),
                vBuffer.begin() + one_record.intel_adr + (idx<<16) + one_record.intel_lg_data,
            ...
            memset(buf, 0, sizeof(buf)) ;
            sprintf(buf, ":%02X%04X%02X", one_record.intel_lg_data, one_record.intel_adr, one_record.intel_type) ;
            out << buf ;

            for(size_t i = 0; i < one_record.intel_lg_data ; ++ i)
            {
                memset(buf, 0, sizeof(buf)) ;
                sprintf(buf, "%02X", (one_record.intel_data[i] & 0xff)) ;
                 out << buf ;
...
bool HexFileToBin(const char* filePath, unsigned char* vOutData,unsigned long* FileSize,unsigned char PaddingByte)
{
    ...
    /* allocate a buffer */
    unsigned char* Memory_Block = (unsigned char*)malloc(MEMORY_SIZE, PadByte) ;
    ...
    do /* repeat until EOF(Filin) */
    {
		...
		if(':' != Line[0] && boEndofFile ==false) return false;//0x0a != Line[0] && NULL != Line[0]) return false;
        ...
        */
		...
		if(Nb_Bytes==0)
			return false;
                ...
                {
                    sscanf (reinterpret_cast<const char *>(p), "%2x",&temp2);
                    p += 2;
        ...
        case 2:
            ...
            if (Seg_Lin_Select == SEGMENTED_ADDRESS)
            {
                sscanf (reinterpret_cast<const char *>(p), "%4x%2x",&Segment,&temp2);
        ...
        case 4:
            ...
            {
                sscanf (reinterpret_cast<const char *>(p), "%4x%2x",&Upper_Address,&temp2);
...
//     fprintf(stdout,"Pad Byte        = %X\n",  PadByte);
    ...
    */
    ...
    {
        memcpy(vOutData,Memory_Block + Lowest_Address, Highest_Address -Lowest_Address+ 1, ) ;
    }
