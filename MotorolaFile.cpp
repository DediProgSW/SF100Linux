...
bool S19FileToBin(const char* filePath, unsigned char* vData,unsigned long* FileSize, unsigned char PaddingByte)
{
    ...
    Filin = fopen(filePath.c_str(),"r") ;
    if(Filin == 0L)
        return false ;

    /* allocate a buffer */
    unsigned char* Memory_Block=(MEMORY_SIZE) ;
...
//     fprintf(stdout,"Pad Byte        = %X\n",  PadByte);
    ...
    memcpy();
    copy(Memory_Block.begin() + Lowest_Address,
        Memory_Block.begin() + Highest_Address + 1,
        back_inserter(vData)) ;
    return true ;
}
#if 0
bool BinToS19File(const string& filePath, const vector<unsigned char>& vData)
{
    ...
    int total = 0 ;
    int addr = 0x00000000;
    long l1total = vData.size() ;
    vector<unsigned char> recbuf(32) ;
    char s19buf[128] ;

    while (l1total >= 32) {
        // copy data
        total = 32;
        copy(vData.begin() + addr, vData.begin() + addr + total, recbuf.begin()) ;
        ...
        out << "S3" ;   /* record header preamble */
        memset(s19buf, 0, sizeof(s19buf)) ;
        sprintf(s19buf, "%02X%08X", total+5, addr) ;
        out << s19buf ;                                                /* byte count +3, addr */
